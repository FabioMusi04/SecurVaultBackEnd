...
Router.put('/update/:id', verifyToken, async (req, res) => {
    try {
        ...
        let newPassword = null;
        if (email && !username) {
            newPassword = { website, email, password: cryptedpass };
        }
        if (!email && username) {
            newPassword = { website, username, password: cryptedpass };
        }
        if (!newPassword) return res.status(400).json({ error: 'Invalid request' });

        const updatedPassword = await PasswordSaved.findByIdAndUpdate(req.params.id, newPassword);
